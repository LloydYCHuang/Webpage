...
如圖4，$$w$$是與這個平面垂直的向量，他的長短尚未決定，但我們需要找到這個$$w$$。既然分類的超平面是$$w^T x + b = 0$$，那麼在平面兩邊的兩組標籤就分別為$$w^T x + b > 0$$及$$w^T x + b < 0$$，代入方程式會有不同的正負予以判別。  
...
數學上為了方便，會再將這個問題變成求得最小的$$\frac{1}{2}\begin{Vmatrix}w\end{Vmatrix}^2$$。  
